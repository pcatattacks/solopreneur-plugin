...
# Some tests removed because skills correctly ask questions in interactive mode,
# but single-turn --print can't verify that. Future: multi-turn harness that
# detects AskUserQuestion calls and feeds scripted answers.
# Removed tests: discover:contextual-vague, design:implicit-visual,
...
get_negative_patterns() {
  ...
  case "$skill" in
    ...
    design)   echo "user flow|visual direction|design-brief|\.solopreneur/designs/|daisyui" ;;
    ...
    ship)     echo "Pre-Launch Checklist|Quality Gate|deployment strategy|preferences.yaml|post-deploy" ;;
    build)    echo "\.solopreneur/plans/|implementation plan|Cursor-ready|build mode" ;;
    kickoff)  echo "Kickoff Report|Consensus|Debate Points|team meeting" ;;
    story)    echo "\.solopreneur/stories/|publishable narrative|observer-log|content-strategist" ;;
    *)        echo "" ;;
  esac
}
