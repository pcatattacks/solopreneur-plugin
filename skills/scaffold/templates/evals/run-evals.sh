...
#   bash evals/run-evals.sh skill1 skill2    # Run a subset of skills
...
#   bash evals/run-evals.sh skill1 skill2 --parallel  # Subset in parallel
#
# Flags:
...
# Find a skill's CSV file (checks both skills/ and .claude/skills/)
find_skill_csv() {
  local skill=$1
  if [ -f "$PLUGIN_DIR/skills/${skill}/eval.csv" ]; then
    echo "$PLUGIN_DIR/skills/${skill}/eval.csv"
  elif [ -f "$PLUGIN_DIR/.claude/skills/${skill}/eval.csv" ]; then
    echo "$PLUGIN_DIR/.claude/skills/${skill}/eval.csv"
  fi
}
...
if [ "${_EVAL_CHILD:-}" = "1" ]; then
  ...
  CSV_FILE=$(find_skill_csv "$SKILL_FILTER")
  if [ -z "$CSV_FILE" ] || [ ! -f "$CSV_FILE" ]; then
    echo "No eval file found for: $SKILL_FILTER" >&2
    exit 1
  fi
...
if [ ${#SKILL_FILTERS[@]} -eq 0 ]; then
  ...
  for _skill in "${SKILL_FILTERS[@]}"; do
    _csv=$(find_skill_csv "$_skill")
    if [ -z "$_csv" ]; then
      echo -e "${RED}No eval file found for: $_skill${NC}"
      exit 1
    fi
...
if [ "$PARALLEL" = true ] && [ "$CSV_COUNT" -gt 1 ]; then
  ...
  for ((batch_start=0; batch_start<TOTAL_SKILLS; batch_start+=MAX_PARALLEL)); do
    ...
    for ((j=batch_start; j<batch_start+MAX_PARALLEL && j<TOTAL_SKILLS; j++)); do
      ...
      wt_dir=$(mktemp -d "${TMPDIR:-/tmp}/eval-sandbox.XXXXXX")
      git -C "$PLUGIN_DIR" branch -q -D "$wt_branch" 2>/dev/null || true
